...
sexratio1 <- sexratio %>% filter(date<'2005-01-01')
sexratio2 <- sexratio %>% filter(date>'2005-01-01',date<'2015-01-01')
sexratio3 <- sexratio %>% filter(date>'2015-01-01')
```
...
Da decidere come mostrare la serie. Ecco varie opzioni.
```{r}
#divisi in 3 plots
p1 <- sr %>% 
...
p2 <- sr %>% 
    filter(date>'2005-01-01',date<'2015-01-01') %>% 
    ggplot(aes(date,perc,group = sesso))+
    geom_bar(stat = 'identity', alpha=0.7, aes(fill=sesso))+
    scale_y_continuous(labels = scales::percent_format())+
    labs(x='',y='')+
    geom_label(data=sexratio2, 
               aes(x=date,y=1.1,
                   label=paste0(ratio,':1')),
               inherit.aes = FALSE,
               angle=90)+
...
p1 + p2 + p3 + plot_layout(widths = c(1/5, 3/5, 2/5))
```
